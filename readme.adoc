...
* Exercise 5.4.4: in the current version, only the agents _obliged_ to vote actually do that. Agents that are _permitted_ to vote do not vote. Change the implementation so that the agents _permitted_ to vote also engage in the voting process. Hints: (i) consider the organizational belief `permission/4`
+
----
permission(A,R,G,D): the agent A is permitted to achieve the goal G with the deadline D because the reason R.
----
